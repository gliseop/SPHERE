...
Восемь встроенных инструментов доступны агентам через `TOOL_DISPATCH` как быстрый путь для типовых бюрократических операций:
...
Помимо встроенных инструментов, агент может описать произвольное действие через `perform_action`. В этом случае LLM-арбитр оценивает допустимость действия в контексте мира (полномочия, ресурсы, локация, социальный граф) и формирует последствия — изменения состояния и побочные эффекты. Этот механизм позволяет агентам предпринимать нестандартные ходы: оказывать неформальное давление, организовывать утечки информации, формировать коалиции — всё, что физически возможно в рамках мира. Подробнее — в [ARCHITECTURE.md](../ARCHITECTURE.md), раздел 20.
...
Агент в MAGISTRY — максимально свободная сущность. Он наблюдает мир, хранит воспоминания, рефлексирует, строит планы и действует исходя из собственной личности и целей. Агент не привязан к фиксированному набору действий: он формулирует намерения, а среда определяет их допустимость и последствия в рамках физики мира.

Когнитивный цикл реализован по модели Park et al. (2023) с расширениями из Park et al. (2024).
