...
Когнитивный агент возвращает JSON-массив freeform-действий (`perform` или `noop`) до `max_actions_per_turn` за ход. Typed actions сохраняются внутри runtime как внутренний слой арбитра и `StateOp`, но не как меню, из которого агент должен выбирать.
...
На уровне когнитивного интерфейса агенту доступны только:

| Действие | Описание |
|---|---|
| `perform` | Свободное действие: описание намерения + опциональная цель |
| `noop` | Осмысленный пропуск хода |

Внутри runtime `actions.py` по-прежнему хранит расширенный набор typed actions (`send_message`, `create_work_item`, `cast_vote`, `request_entity` и т.д.), но теперь они рассматриваются как внутренний исполнительный словарь арбитра и совместимый слой тестов/runtime, а не как пользовательский интерфейс когнитивного агента.

Каждое действие по-прежнему может содержать `justification` — краткое обоснование мотива.
...
Базовая коммуникация больше не трактуется как отдельная permission-gated capability. Для сообщений арбитр проверяет не `message`-capability, а физику мира: корректность typed-id, тип цели и, для приватного контакта, совместимость пространственного положения агентов. Если у обеих сторон задан `zone_id`, приватный контакт возможен только при совпадающей зоне.

Для legacy/runtime `spawn_agent` дополнительно проверяются `runtime.allow_runtime_spawn`, лимит `runtime.max_agents`, отсутствие конфликта по `agent:{slug}`, temporal-validation по абсолютным датам и безопасный набор capabilities (`message`/`work`). Арбитр также отклоняет self-nomination, self-vote цели голосования и role-based display-name для новых агентов.
