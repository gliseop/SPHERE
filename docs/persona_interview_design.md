...
Параллелизация самой симуляции (этап действий агентов) реализована через параметр `parallel_window_seconds`: агенты, чьё время пробуждения отличается не более чем на заданное окно (в симулированных секундах), объединяются в батч. Решения внутри батча генерируются параллельно через `asyncio.gather`, а применение к миру — последовательно. Рекомендуемое значение окна — 300 секунд (5 симулированных минут).
