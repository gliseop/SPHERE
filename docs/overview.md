...
**Полномочие** (`Capability`) — право агента совершать определённое действие через встроенные инструменты. Не роль, а конкретное право: `open_case` (открывать дела), `submit_proposal` (подавать предложения), `resolve_case` (принимать решения), `audit` (проверять), `vote` (голосовать). Один агент может обладать несколькими полномочиями. За пределами встроенных инструментов агент может описать произвольное действие, оцениваемое LLM-арбитром в контексте физики мира.
