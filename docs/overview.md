...
```mermaid
C4Context
    title Архитектура SPHERE

    System_Boundary(engine, "Движок симуляции (sphere_lc)") {
...
Движок симуляции (`src/sphere_lc/`) содержит всю логику: оркестрацию ходов, агентов, арбитра, модели данных и метрики. Сервер (`web/backend/`) предоставляет REST API для управления прогонами и WebSocket для потоковой передачи событий. Клиент (`web/frontend/`) визуализирует социальный граф, ленту событий и детали агентов.
...
**Действие** (`Action`) — внутренний typed vocabulary runtime: `send_message`, `add_work_note`, `submit_work_proposal`, `cast_vote`, `spawn_agent`, `perform` и др. Когнитивный агент больше не выбирает эти типы напрямую: он формулирует свободный `proposal`, а арбитр уже материализует его во внутренние действия мира.

**Полномочие** (`Capability`) — право агента совершать определённый класс действий: `message` (legacy/internal совместимость коммуникационных typed-ops), `work` (рабочие записи и предложения), `dao` (голосование), `spawn` (legacy/runtime-введение новых участников). Один агент может обладать несколькими полномочиями. Сюжетный capability `audit` больше не используется: аудит вынесен в отдельный runtime-layer.

**Арбитр** (`Arbiter`) — гибридный механизм: для legacy typed actions он делает детерминированную проверку полномочий и существования адресатов, а основной когнитивный путь проходит через LLM-materialization свободного `proposal` на основе YAML-журнала мира.
