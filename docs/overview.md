...
**Действие** (`Action`) — структурированная команда агента: `send_message`, `add_work_note`, `submit_proposal`, `cast_vote`, `perform` (произвольное). Каждое действие проходит через арбитр, который проверяет полномочия и валидность.

**Полномочие** (`Capability`) — право агента совершать определённый класс действий: `message` (коммуникация), `work` (рабочие записи и предложения), `dao` (голосование), `audit` (проверка). Один агент может обладать несколькими полномочиями.
