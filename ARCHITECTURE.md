...
## 6. Восемь инструментов

Набор инструментов минимален: один для общения, один для перемещения, шесть для изменения состояния мира. Четыре из шести — универсальные, работают с делами любого типа. Остальную информацию агент получает текстом в ситуационной сводке.
...
**Действие**: LLM получает ситуацию, план и релевантные воспоминания, затем выбирает инструменты для исполнения. Поддерживаются все восемь инструментов, включая `talk_to`, `open_case`, `submit_proposal`, `resolve_case`, `file_report`, `cast_vote`, `add_note`, `move_to`.
