...
[docs/chapters/chapter_1.md](docs/chapters/chapter_1.md) — первая глава ВКР, содержащая обзор литературы и обоснование подхода. Глава охватывает проблему принципала-агента в иерархических организациях (Йенсен, Меклинг), четыре категории подходов к снижению агентских издержек (институциональные, технологические, поведенческие, децентрализованные), эволюцию агентного моделирования от детерминированных правил к БЯМ-агентам, а также формулирует исследовательский пробел — отсутствие инструмента для экспериментальной оценки гибридных управленческих механизмов в среде с адаптивными агентами. Раздел 1.6 описывает архитектуру предлагаемой гибридной системы (ИИ-аудитор, репутационный механизм, коллегиальное рассмотрение), включая границы применимости и обоснование проектных решений.

[docs/chapters/chapter_2.md](docs/chapters/chapter_2.md) — вторая глава ВКР, посвящённая проектированию платформы SPHERE. Глава обосновывает методологию проектного исследования (design science research), описывает общую архитектуру модельной среды, когнитивную модель агента, трёхэтапную цепочку формирования личности (затравка, профиль HEXACO/Dark Triad как authoring-слой, нарративное интервью), систему полномочий и действий, четыре конфигурируемых режима управления (G0–G3), трёхуровневую систему верификации (детерминированная истина, метрики эффективности, правдоподобие) и план эксперимента.

[docs/concept.md](docs/concept.md) — концепция исследования в чистом виде, без привязки к реализации: проблема контроля, три компонента гибридной системы (ИИ-аудитор, накопительная репутация, коллегия), принципы проектирования и границы применимости, методология проверки в агентной симуляции (асимметрия знания трёх наблюдателей, лестница режимов G0–G3, проверка воздействия), гипотезы, интерпретационная рамка исходов и трёхуровневая система оценки.
...
Runtime-аудитор и его развитие — в [docs/auditor_refactor_plan.md](docs/auditor_refactor_plan.md).
...
│   ├── auditor_refactor_plan.md # План развития runtime-аудитора
│   ├── chapters/               # Главы исследовательской работы
│   │   ├── chapter_1.md
│   │   ├── chapter_2.md
│   │   └── chapter_3.md
...
│   ├── concept.md              # Концепция гибридной системы управления и её проверки (без привязки к реализации)
...
2. **Полномочия вместо ролей.** Агент определяется набором полномочий, а не жёсткой ролью. В актуальной модели предметно значимыми capability-гейтами остаются прежде всего `work`, `dao` и legacy/runtime `spawn`; базовая коммуникация ограничивается физикой мира и адресацией, а не отдельным permission-гейтом.

3. **Агенты максимально свободны.** Агент — автономная сущность с собственными целями и личностью. Интерфейс когнитивного агента должен оставаться freeform: агент описывает намерение как свободное действие, а арбитр уже переводит его в детерминированные операции над состоянием. Среда обеспечивает физику, но не диктует поведение.

4. **Текстовые описания вместо числовых параметров.** Личность агента задаётся биографией и текстовой характеристикой, а не числовыми шкалами (обосновано в [docs/chapters/chapter_1.md](docs/chapters/chapter_1.md), раздел 1.4, по результатам Park et al. [28]).
